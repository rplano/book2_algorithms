...
/**
 * The <code>GQuadLine</code> class is a graphical object whose appearance consists
 * of a quad-line segment.
 */
public class GQuadLine extends GObject implements GScalable {
	 ...
	 */
	public GQuadLine(double x0, double y0, double xm, double ym, double x1, double y1) {
		setLocation(x0, y0);
		dx = x1 - x0;
		dy = y1 - y0;
		dxm = x1 - xm;
	...
	public void paint(Graphics g) {
		...
		path.moveTo(GMath.round(x), GMath.round(y));
		path.quadTo(GMath.round(x + dxm),GMath.round(y + dym), GMath.round(x + dx),GMath.round(y + dy));
	...
	public GRectangle getBounds() {
		...
		return new GRectangle(x, y, Math.abs(dx) + 1, Math.abs(dy) + 1);
	}
	 ...
	 * exists only to provide symmetry with <a
	 * href="#setStartPoint(double, double)"><code>setStartPoint</code></a>.
	 *
	...
	public GPoint getMiddlePoint() {
		return new GPoint(getX() + dxm, getY() + dym);
	}	
	...
	public boolean contains(double x, double y) {
		...
		if (distanceSquared(x, y, x1, y1) < tSquared)
			return true;
		if (x < Math.min(x0, x1) - LINE_TOLERANCE)
			return false;
		if (x > Math.max(x0, x1) + LINE_TOLERANCE)
			return false;
		if (y < Math.min(y0, y1) - LINE_TOLERANCE)
			return false;
		if (y > Math.max(y0, y1) + LINE_TOLERANCE)
			return false;
	...
	/**
	 * @inherited GObject#boolean contains(GPoint pt) Checks to see whether a
	 *            point is inside the object.
	 */

	/* Inherited method: sendToFront() */
	/**
	 * @inherited GObject#void sendToFront() Moves this object to the front of
	 *            the display in the <i>z</i> dimension.
	 */

	/* Inherited method: sendToBack() */
	/**
	 * @inherited GObject#void sendToBack() Moves this object to the back of the
	 *            display in the <i>z</i> dimension.
	 */

	/* Inherited method: sendForward() */
	/**
	 * @inherited GObject#void sendForward() Moves this object one step toward
	 *            the front in the <i>z</i> dimension.
	 */

	/* Inherited method: sendBackward() */
	/**
	 * @inherited GObject#void sendBackward() Moves this object one step toward
	 *            the back in the <i>z</i> dimension.
	 */

	/* Inherited method: setColor(color) */
	/**
	 * @inherited GObject#void setColor(Color color) Sets the color used to
	 *            display this object.
	 */

	/* Inherited method: getColor() */
	/**
	 * @inherited GObject#Color getColor() Returns the color used to display
	 *            this object.
	 */

	/* Inherited method: setFilled(fill) */
	/**
	 * @inherited GObject#void setFilled(boolean fill) Sets whether this object
	 *            is filled.
	 */

	/* Inherited method: isFilled() */
	/**
	 * @inherited GObject#boolean isFilled() Returns whether this object is
	 *            filled.
	 */

	/* Inherited method: setVisible(visible) */
	/**
	 * @inherited GObject#void setVisible(boolean visible) Sets whether this
	 *            object is visible.
	 */

	/* Inherited method: isVisible() */
	/**
	 * @inherited GObject#boolean isVisible() Checks to see whether this object
	 *            is visible.
	 */

	/* Inherited method: addMouseListener(listener) */
	/**
	 * @inherited GObject#void addMouseListener(MouseListener listener) Adds a
	 *            mouse listener to this graphical object.
	 */
	...
	/**
	 * @inherited GObject#void addMouseMotionListener(MouseMotionListener
	 *            listener) Adds a mouse motion listener to this graphical
	 *            object.
	 */

	/* Inherited method: removeMouseMotionListener(listener) */
	/**
	 * @inherited GObject#void removeMouseMotionListener(MouseMotionListener
	 *            listener) Removes a mouse motion listener from this graphical
	 *            object.
	 */
